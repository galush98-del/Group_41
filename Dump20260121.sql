...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `flightbookings_generate_code_before_insert` BEFORE INSERT ON `flightbookings` FOR EACH ROW BEGIN
  DECLARE new_code VARCHAR(200);
  DECLARE tries INT DEFAULT 0;
  DECLARE cnt INT DEFAULT 0;

  -- רק אם לא סופק קוד (או נשאר ברירת מחדל ריקה)
  IF NEW.booking_code IS NULL OR NEW.booking_code = '' THEN
    ...
    REPEAT
      ...
      FROM FlightBookings
      WHERE booking_code = new_code;

    UNTIL cnt = 0 OR tries >= 10
    END REPEAT;

    IF cnt <> 0 THEN
      SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Failed to generate unique booking_code after 10 attempts.';
    ELSE
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `flights_set_arrival_time_before_insert` BEFORE INSERT ON `flights` FOR EACH ROW BEGIN
  DECLARE dur INT;

  SELECT total_duration_minutes
    INTO dur
  FROM FlightDurations
  WHERE origin_airport_name = NEW.origin_airport_name
    AND dest_airport_name   = NEW.dest_airport_name
  LIMIT 1;

  IF dur IS NULL THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'No matching duration found in FlightDurations for this route.';
  ELSE
    SET NEW.arrival_time = DATE_ADD(NEW.departure_time, INTERVAL dur MINUTE);
  END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `prevent_airplane_overlap_before_insert` BEFORE INSERT ON `flights` FOR EACH ROW BEGIN
    IF EXISTS (
        SELECT 1
        FROM Flights f
        WHERE f.airplane_id = NEW.airplane_id
          AND NEW.departure_time < f.arrival_time
          AND NEW.arrival_time > f.departure_time
    ) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Airplane is already assigned to an overlapping flight.';
    END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `flights_set_arrival_time_before_update` BEFORE UPDATE ON `flights` FOR EACH ROW BEGIN
  DECLARE dur INT;

  IF (NEW.departure_time <> OLD.departure_time)
     OR (NEW.origin_airport_name <> OLD.origin_airport_name)
     OR (NEW.dest_airport_name <> OLD.dest_airport_name) THEN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `prevent_airplane_overlap_before_update` BEFORE UPDATE ON `flights` FOR EACH ROW BEGIN
    IF EXISTS (
        SELECT 1
        FROM Flights f
        WHERE f.airplane_id = NEW.airplane_id
          AND f.flight_no <> NEW.flight_no
          AND NEW.departure_time < f.arrival_time
          AND NEW.arrival_time > f.departure_time
    ) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Airplane overlap detected on update.';
    END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
--
-- Dumping routines for database 'fly_tau_db'
--
...
-- Dump completed on 2026-01-21 18:36:40
